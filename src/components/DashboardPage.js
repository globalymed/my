...
const DashboardPage = () => {
  ...
  // Dialog states
  // State for enlarged appointment card (removed)
  ...
  // Dropdown open state for appointment cards
  const [openDropdownId, setOpenDropdownId] = useState(null);

  // Editable notes state: { [appointmentId]: string }
  const [editableNotes, setEditableNotes] = useState({});
  const [notesSaving, setNotesSaving] = useState({});
  const [notesError, setNotesError] = useState({});
  // Track which appointment's notes are being edited
  const [editingNotesId, setEditingNotesId] = useState(null);

  // Save notes to Firestore
  const handleSaveNotes = async (appointmentId) => {
    const newNote = editableNotes[appointmentId] || "";
    setNotesSaving((prev) => ({ ...prev, [appointmentId]: true }));
    setNotesError((prev) => ({ ...prev, [appointmentId]: null }));
    try {
      const appointmentRef = doc(db, "appointments", appointmentId);
      await updateDoc(appointmentRef, { notes: newNote, updatedAt: serverTimestamp() });
      setNotesSaving((prev) => ({ ...prev, [appointmentId]: false }));
      // Optionally, refresh appointments
      if (user && user.id) fetchUserAppointments(user.id);
    } catch (err) {
      setNotesSaving((prev) => ({ ...prev, [appointmentId]: false }));
      setNotesError((prev) => ({ ...prev, [appointmentId]: "Failed to save note." }));
    }
  };
          ...
          if (parsedUser.id) {
            try {
              ...
              if (userDocSnap.exists()) {
                ...
                console.warn(
                  `No user found with ID: ${parsedUser.id}, using localStorage data`
                );
              ...
              console.error(
                "Error fetching user from Firestore directly:",
                err
              );
    ...
    try {
      ...
      if (selectedAppointment.availabilityId) {
        await deleteAppointment(
          selectedAppointment.id,
          selectedAppointment.availabilityId
        );
      }
        ...
        >
          <CardContent sx={{ p: 4 }}>
            ...
            <Typography variant="body1" color="text.secondary" sx={{ mb: 3 }}>
              Please log in to view your appointment details and manage your
              healthcare journey.
            </Typography>
            <Box
              sx={{
            ...
            >
              <Typography variant="body2" sx={{ fontStyle: "italic", mb: 1 }}>
                New to MedYatra? Book an appointment to receive your login
  ...
  return (
    ...
    >
      <Box sx={{ py: 4 }}>
        ...
        {/* Removed Book Appointment and Health Assistant buttons as requested */}
        {/* Adjust margin below to compensate for removed toolbar */}
        <Box sx={{ height: 24 }} />
        ...
        {/* Appointments Grid with dropdown details and margin management */}
        {!appointmentLoading && appointments.length > 0 && (
          <Grid container spacing={3}>
            {appointments.map((appointment, idx) => {
              ...
              const isOpen = openDropdownId === appointment.id;

              // No margin on Grid item; add margin only inside expanded card
              ...
              return (
                <Grid item xs={12} md={6} key={appointment.id}>
                  <Card
                    ...
                    onClick={() => setOpenDropdownId(isOpen ? null : appointment.id)}
                  >
                    <CardContent sx={{ p: 3 }}>
                      <Box
                        sx={{
                      ...
                      {/* Notes section: always visible, editable on click */}
                        ...
                        {editingNotesId === appointment.id ? (
                          <>
                            <TextField
                              ...
                              size="small"
                              value={
                                editableNotes[appointment.id] !== undefined
                                  ? editableNotes[appointment.id]
                                  : appointment.notes || ""
                              }
                              onChange={(e) => {
                                setEditableNotes((prev) => ({ ...prev, [appointment.id]: e.target.value }));
                              }}
                              disabled={notesSaving[appointment.id]}
                            ...
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mt: 1 }}>
                              <Button
                                ...
                                onClick={(e) => {
                                  e.stopPropagation();
                                  handleSaveNotes(appointment.id);
                                  setEditingNotesId(null);
                                }}
                                sx={{ borderRadius: 2, textTransform: 'none', minWidth: 80 }}
                              >
                                {notesSaving[appointment.id] ? 'Saving...' : 'Save'}
                              </Button>
                              <Button
                                variant="outlined"
                                size="small"
                                ...
                                onClick={(e) => {
                                  e.stopPropagation();
                                  setEditingNotesId(null);
                                  setEditableNotes((prev) => ({ ...prev, [appointment.id]: appointment.notes || "" }));
                                }}
                                sx={{ borderRadius: 2, textTransform: 'none', minWidth: 80 }}
                              >
                                Cancel
                              </Button>
                              {notesError[appointment.id] && (
                                <Typography color="error" variant="caption">{notesError[appointment.id]}</Typography>
                              )}
                            </Box>
                          </>
                        ) : (
                          ...
                          >
                            <Typography variant="body2" color="text.secondary" sx={{ fontStyle: "italic" }}>
                              {appointment.notes ? appointment.notes : <span style={{ color: '#aaa' }}>Click to add notes...</span>}
                            </Typography>
                          </Box>
                        )}
                      </Box>
                      ...
                      {/* Dropdown details */}
                      {isOpen && (
                        <Box sx={{ mt: 2, mb: 1, p: 2, bgcolor: "#f1f5f9", borderRadius: 2, boxShadow: '0 2px 8px rgba(0,0,0,0.07)' }}>
                          ...
                          <Typography variant="body2" sx={{ mb: 0.5 }}>
                            <strong>Treatment Type:</strong> {treatmentType || 'Consultation'}
                          </Typography>
                        </Box>
                      )}
                          ...
                          {status.toLowerCase() !== "cancelled" && status.toLowerCase() !== "completed" && (
                            <Tooltip title="Mark as Completed">
                              <IconButton
                                ...
                                onClick={(e) => {
                                  e.stopPropagation();
                                  handleStatusDialogOpen(appointment, "completed");
                                }}
                              >
                                <CheckIcon />
                              </IconButton>
                            ...
                            <>
                              ...
                              <Tooltip title="Cancel Appointment">
                                <IconButton
                                  ...
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    handleCancelDialogOpen(appointment);
                                  }}
                                >
                                  <DeleteIcon />
                                </IconButton>
              ...
              <Box sx={{ my: 2 }}>
                <Typography variant="body2" gutterBottom>
                  ...
                  {selectedAppointment.doctorName ||
                    "Dr. " + (selectedAppointment.clinicName || "Unknown")}
                </Typography>
                <Typography variant="body2" gutterBottom>
                  <strong>Date:</strong>{" "}
                ...
                <Typography variant="body2" gutterBottom>
                  <strong>Time:</strong>{" "}
                  {selectedAppointment.timeSlot || "Not specified"}
                </Typography>
              </Box>
            )}
            <TextField
